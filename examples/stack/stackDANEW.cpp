...
struct stack
{
    item *data = new item[CAPACITY]; // the actual data
    size_t capacity{CAPACITY};       // actual capacity of the stack (max number of elements)
...
bool push(stack &s, const item &i)
{
    ...
    if (!result)
    {
        ...
        size_t newCapacity = capacity(s) * 2;
        ...
        { // success
            ...
            delete[] s.data; // get rid of old data
            s.data = newData;
