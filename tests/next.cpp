...
un’opportuna funzione “average” a ciascuno dei valori nelle posizioni da 0 a
N – S - 1 la media aritmetica tra il valore stesso e gli S (S > 0) valori successivi.
Ad esempio, dato N = 6, S = 1 e l’array v: [1, 3, 2, 5, 7, 9], la funzione deve
modificare l’array riducendolo v: [2, 2.5, 3.5, 6, 8, 9]; infatti:
◦ nuovo v[0] = (v[0] + v[1]) / 2 = (1 + 3) / 2 = 2
◦ nuovo v[1] = (v[1] + v[2]) / 2 = (3 + 2) / 2 = 2.5
◦ nuovo v[2] = (v[2] + v[3]) / 2 = (2 + 5) / 2 = 3.5
e così via...
Ad esempio, dato N = 6, S = 2 e l’array v: [1, 3, 2, 5, 7, 9], la funzione deve
modificare l’array riducendolo v: [2, 3.333, 4.666, 7, 7, 9]; infatti:
◦ nuovo v[0] = (v[0] + v[1] + v[2]) / 3 = (1 + 3 + 2) / 3 = 2
◦ nuovo v[1] = (v[1] + v[2] + v[3]) / 3 = (3 + 2 + 5) / 3 = 3.333
◦ nuovo v[2] = (v[2] + v[3] + v[4]) / 3 = (2 + 5 + 7) / 3 = 4.666
e così via...
...
/// @brief a[i] = average of a[i], ..., a[i + S]
/// @param a   the array to process
/// @param N   numero di elementi
/// @param S numero di successivi da considerare
void average(type a[], int N, int S)
{
    type sum = 0;
    for (int i = 0; i < S; ++i)
