...
void powers(int a[], int base, int dim);
che assegni agli elementi dell’array a con indice da 0 a dim – 1 i valori delle
successive potenze intere del valore base in modo tale che a[indice] risulti pari
a baseindice.
Ad es. dato l’array dichiarato come
int potenze[10];
dopo la chiamata alla funzione powers seguente
powers(potenze, 2, 10);
l’array potenze deve risultare uguale a quello così inizializzato
int pot[10] {1, 2, 4, 8, 16, 32, 64, 128, 256, 512};
ovvero contenere i valori delle prime 10 potenze di 2, in ordine crescente.
...
Assunzione: -100 <= base <= 100, base <> 0, 1 <= numero elementi <= 10.
...
void powers(int a[], int base, int dim, bool efficient = true);
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        base = getIntInRange("Immetti la base: ", -100, 100);
    }
    while(base == 0);

    int dim = getIntInRange("Immetti il numero di elementi: ", 1, 10);

//    cout << base << dim << endl;

    int potenze[dim];

    powers(potenze, base, dim);
